...
var localaddress = "3jui8ko762DHguaNcfoHgJkWvCWsUo4ok";
var remoteAddress = "C6MbP6yWiAi6rfRo8KBWrCv6tVW9q3r5D";
var dchat = new bnrtc2_dchat_node_1.Dchat("127.0.0.1", 19999);
...
http.createServer(function (req, res) {
    ...
    if (req.method == "GET") {
        ...
        if (url.pathname === "/localaddress") {
            ...
            dchat.send(remoteAddress, DCHAT_DPORT, buf.data()).then(function (res) {
                if (res == bnrtc2_dchat_typings_1.MessageState.Success) {
                    logIt("send data: ", message_1, remoteAddress, "ok");
                }
                else {
                    logIt("send data: ", message_1, remoteAddress, "failed", bnrtc2_dchat_typings_1.MessageState[res]);
                }
            });
        }
        return res.end();
